...
// Edge labels
schema.edgeLabel('hasAddress').single().connection('customer', 'address').ifNotExists().create()
schema.edgeLabel('places').single().connection('customer', 'order').ifNotExists().create()
schema.edgeLabel('usesCard').single().connection('order', 'creditCard').ifNotExists().create()
schema.edgeLabel('logsInto').single().connection('customer', 'session').ifNotExists().create()
schema.edgeLabel('chargedWith').single().connection('customer', 'chargeback').ifNotExists().create()
schema.edgeLabel('fromCard').single().connection('chargeback', 'creditCard').ifNotExists().create()
schema.edgeLabel('resultsIn').single().connection('order', 'chargeback').ifNotExists().create()
schema.edgeLabel('using').single().connection('session', 'device').connection('order', 'device').ifNotExists().create()
